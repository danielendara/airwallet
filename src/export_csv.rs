...
pub fn write_csv_ledger(path: &PathBuf, wallets: &[Wallet]) -> Result<PathBuf, String> {
    ...
    for wallet in wallets {
        for row in wallet.ledger_rows_sorted(LedgerSort::OldestFirst) {
            csv.push_str(&csv_field(&wallet.child_name));
            csv.push(',');
            csv.push_str(&csv_field(&row.date.label()));
            csv.push(',');
            csv.push_str(&csv_field(row.description));
            csv.push(',');
            csv.push_str(&csv_field(&format_money_input(row.amount_cents)));
            csv.push(',');
            csv.push_str(&csv_field(&format_money_input(row.balance_cents)));
            csv.push_str("\r\n");
        }
    }
